...
def import_from_path(path: Path) -> ModuleType:
    ...
    return module
# ======================================================================
def _insert_option(option_tree: dict[str, OptionTree | str], option_name: str) -> None:
    options = [option_name]
...
def search_versions(pypi_name: str) -> Version | None:
    ...
    # return versions
# ======================================================================
def _package_dependencies(path_project: Path,
                          limepackage_info: dict[str, Any],
                          pypi_name: str):
    meta = limepackage_info.get('dependencies', {})

    path_dependencies = (path_project
                         / Path(meta.get('path', DEFAULT_DEPENDENCIES_PATH)))
    ...
    return {option: {'file': _path.relative_to(path_project).as_posix()}
            for option, _path in sorted(all_options.items(),
                                        key = lambda item: item[0])}
# ----------------------------------------------------------------------
def _package_readme(path_project: Path,
                    pyproject: dict[str, Any],
                    project_info: dict[str, Any],
                    limepackage_info: dict[str, Any],
                    ref: str):
    urls: dict[str, Any] = _get(project_info, 'urls', default_factory = dict)
    source_url = _get(urls, 'Source Code',
                      default_factory = lambda: urls.get('Homepage', ''))
    path_readme_text: Path = path_project / _get(project_info, 'readme', 'README.md')

    path_readme_generator = path_project / _get(limepackage_info,
                                                'readme', DEFAULT_README_GENERATOR_PATH)
    try:
        readme_text = str(import_from_path(path_readme_generator).main(pyproject)) + '\n'
...
def _get(_dict: dict,
    ...
    return value
# ======================================================================
def package(path: Path | None = None, *,
            build: bool = False,
    ...
    tool_info: dict[str, Any] = _get(pyproject, 'tool',
                                          default_factory = dict)
    limepackage_info: dict[str, Any] = _get(tool_info, 'limepackage',
                                                 default_factory = dict)
    ...
    _get(_get(tool_info, 'setuptools',
              default_factory = dict
              ), 'dynamic',
         default_factory = dict
         )['optional-dependencies'] = _package_dependencies(path,
                                                            limepackage_info,
                                                            pypi_name)
    # ------------------------------------------------------------------
    # Readme
    (path_readme_text,
     readme_text,
     readme_text_pypi) = _package_readme(path,
                                         pyproject,
                                         project_info,
                                         limepackage_info,
                                         ref)
    ...
    if pre_release is not None: # Pre-release
        if pre_release[-1].isdigit():
            ...
            if (latest is None
                ...
                project_info['version'] = f'{project_info['version']}{pre_release}1'
            else:
                project_info['version'] = f'{project_info['version']}{pre_release}{latest.pre[1]}'

    # ------------------------------------------------------------------
    if dev_release is not None: # Developmental release
        project_info['version'] += f'.dev{time_ns() >> 30 if dev_release == '' else dev_release}'
    # ------------------------------------------------------------------
    # RUNNING THE BUILD
