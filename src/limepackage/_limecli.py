...
if TYPE_CHECKING:
    ...
    Callable = Generator = Sequence = tuple
    ModuleType = object
    Any = Self = object
...
ArgType = GenericAlias | UnionType | type
Arg = FromStr | GenericAlias | UnionType | EnumType
...
class NamedTupleLike(Protocol):
    ...
    __annotations__: dict[str, type[Arg]]
...
# ----------------------------------------------------------------------
def _typename(argtype: str | type[Arg]) -> str:
    """Gives a more readable name of the type if available."""

    return (
        argtype.__name__
        if (
            hasattr(argtype, '__name__')
...
# ----------------------------------------------------------------------
def _format_error(
    arg: str, argtype: str | type[Arg], exc: str | Exception
) -> str:
...
def _namedtuple(arg: str, argtype: NamedTupleLike) -> NamedTupleLike:
    ...
    return argtype._make(
        _convert_type(_arg, _argtype)
        for _arg, _argtype in zip(
            arg.split(','), argtype.__annotations__.values()
        )
    )
...
def _tuple(
    args: list[str],
    argtypes: tuple[type[Arg], ...] | tuple[type[Arg], EllipsisType],
    tuplelike: type,
) -> tuple[Any]:
    """Handles a tuple."""
    ...
    return tuplelike(
        _convert_type(arg, argtype) for arg, argtype in zip(args, argtypes)
    )
...
# ----------------------------------------------------------------------
def _convert_pair[Key: Arg, Value: Arg](
    arg: str, keytype: type[Key], valuetype: type[Value]
) -> tuple[Key, Value] | tuple[None, list[str]]:
    ...
    while (partitioning := arg.partition(':'))[1]:
        ...
        try:
            return (  # type: ignore[return-value]
                _convert_type(
                    keyarg[1:-1] if is_keyparens else keyarg, keytype
                ),
                _convert_type(arg[1:-1] if is_valueparens else arg, valuetype),
            )
...
# ----------------------------------------------------------------------
def _dict[Key: Arg, Value: Arg](
    arg: str, keytype: type[Key], valuetype: type[Value], dictlike: type
) -> dict[Key, Value]:
    """Handles a dictionary."""
    if not _is_parenthesis(arg):
        raise TypeConversionError(
            f"argument '{arg}' must start with" "'(' and end with ')'"
        )
    ...
    while subargs:
        ...
        pair: tuple[Key, Value] | tuple[None, list[str]] = _convert_pair(
            subarg, keytype, valuetype
        )
        if pair[0] is None:
            errormessages.append(
                _format_error(
                    ...
                    '\n'.join(pair[1]),  # type: ignore[arg-type]
                )
            )
            continue
        ...
        errormessages.append(
            f"{subarg} --> {_typename(keytype)}: {_typename(valuetype)}"
        )
        subarg = ''
    ...
    try:
        ...
        for member_name, member in argtype.__members__.items():
            if member_name.casefold() == arg_cf:
                return member
    raise TypeConversionError(
        f"{arg} not in members of {argtype.__name__} "
...
def _convert_type(arg: str, argtype: type[Arg]) -> object:
    ...
    try:
        ...
        raise TypeConversionError(
            _format_error(arg, argtype, exc)
        )  # pylint: disable=raise-missing-from
    try:  # Direct conversion
        return argtype(arg)  # type: ignore[call-arg, arg-type]
...
# ----------------------------------------------------------------------
def _make_function_helpstring(name: str, function: FunctionType):
...
def _make_helpstring(module: ModuleType) -> str:
    ...
    # Generating helptext
    return f'Functions available: {'\n\n'.join([_make_function_helpstring(name, function) for name, function in cli_hooks(module)])}'
...
def _convert_parameter(arg: str, parameter: Parameter):
    ...
    try:
        ...
        raise TypeConversionError(
            f"Converting input for '{parameter.name}'"  # pylint: disable=raise-missing-from
            f" failed.\n{exc}"
        )


# ----------------------------------------------------------------------
def _convert_args(args: Sequence[str], function: FunctionType) -> ArgsKwargs:
    ...
    for parameter in function_signature.parameters.values():
        if var_pos_not_found and parameter.kind == Parameter.VAR_POSITIONAL:
    ...
    for parameter_name, cli_arg in cli_kwargs.items():
        ...
        converted_kwargs[parameter_name] = _convert_parameter(
            cli_arg, parameter
        )
        # print(parameter_name, converted_kwargs[parameter_name])
    converted_args = [
        _convert_parameter(cli_arg, parameter)
        for cli_arg, parameter in zip(
            cli_args, chain(parameters.values(), repeat(parameter_var_pos))
        )
    ]
...
def function_cli(
    ...
    if (function is None and not args) or '--help' in args:
        print(
            _make_helpstring(module)
            if function is None
            else _make_function_helpstring(function.__name__, function)
        )
        return 0
    ...
    if function is None:
        try:
            ...
            print(
                f"{str(module)[1:-1].capitalize()} does not have a function '{args[0]}'"
            )
            return 2
...
def get_main(
    ...
    return partial(
        function_cli, function=function, module=module, package=package
    )
